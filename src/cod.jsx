...
44,entrepreneur,single,tertiary,no,1136,yes,yes,cellular,3,apr,345,2,249,7,other,no`;
  ...
  const heartAttributes = {
    ...
    cp: { name: 'Tipo de Dor no Peito', type: 'categorical', values: { '1.0': 'Tipo 1', '2.0': 'Tipo 2', '3.0': 'Tipo 3', '4.0': 'Tipo 4' } },
    trestbps: { name: 'Pressão Arterial', type: 'numeric', min: 90, max: 200 },
    chol: { name: 'Colesterol', type: 'numeric', min: 100, max: 400 },
    fbs: { name: 'Açúcar no Sangue', type: 'categorical', values: { '0.0': 'Não', '1.0': 'Sim' } },
    restecg: { name: 'ECG Repouso', type: 'categorical', values: { '0.0': 'Normal', '1.0': 'Anormal', '2.0': 'Hipertrofia' } },
    thalach: { name: 'Freq. Cardíaca Máx', type: 'numeric', min: 70, max: 210 },
    exang: { name: 'Angina Exercício', type: 'categorical', values: { '0.0': 'Não', '1.0': 'Sim' } },
    oldpeak: { name: 'Depressão ST', type: 'numeric', min: 0, max: 7, step: 0.1 },
    slope: { name: 'Inclinação ST', type: 'categorical', values: { '1.0': 'Ascendente', '2.0': 'Plana', '3.0': 'Descendente' } },
    ca: { name: 'Vasos Coloridos', type: 'categorical', values: { '0.0': '0', '1.0': '1', '2.0': '2', '3.0': '3' } },
  ...
  const bankAttributes = {
    ...
    marital: { name: 'Estado Civil', type: 'categorical', values: { 'divorced': 'Divorciado', 'married': 'Casado', 'single': 'Solteiro' } },
    education: { name: 'Educação', type: 'categorical', values: { 'primary': 'Primário', 'secondary': 'Secundário', 'tertiary': 'Superior' } },
    default: { name: 'Inadimplente', type: 'categorical', values: { 'no': 'Não', 'yes': 'Sim' } },
    balance: { name: 'Saldo', type: 'numeric', min: -10000, max: 100000 },
    housing: { name: 'Emprést. Imob.', type: 'categorical', values: { 'no': 'Não', 'yes': 'Sim' } },
    loan: { name: 'Emprést. Pessoal', type: 'categorical', values: { 'no': 'Não', 'yes': 'Sim' } },
    contact: { name: 'Tipo de contato', type: 'categorical', values: { 'cellular': 'Celular', 'telephone': 'Telefone', 'unknown': 'Desconhecido' } },
    day: { name: 'Dia', type: 'numeric', min: 1, max: 31 },
    month: { name: 'Mês', type: 'categorical', values: { 'jan': 'Janeiro', 'feb': 'Fervereiro', 'mar': 'Março', 'apr': 'Abril', 'may': 'Maio', 'jun': 'Junho', 'jul': 'Julho', 'aug': 'Agosto', 'sep': 'Setembro', 'oct': 'Outubro', 'nov': 'Novembro', 'dec': 'Dezembro' } },
    campaign: { name: 'Qtd. de contatos nesta campanha', type: 'numeric', min: 1, max: 100 },
    pdays: { name: 'Dias desde o último contato (-1 caso não tenha feito)', type: 'numeric', min: -1, max: 1000 },
    previous: { name: 'Contatos anteriores', type: 'numeric', min: 0, max: 100 },
  ...
  const discretizeNumeric = (value, allValues) => {
    ...
    if (num <= q1) return 'baixo';
    if (num <= q2) return 'medio';
    return 'alto';
  };
  ...
  const calculateNaiveBayes = () => {
    ...
    const classIndex = data[0].length - 1;
    const classes = [...new Set(data.map(row => row[classIndex]))];
    
    const classCounts = {};
    classes.forEach(c => { classCounts[c] = 0; });
    data.forEach(row => { classCounts[row[classIndex]]++; });
    
    const classProbs = {};
    classes.forEach(c => { classProbs[c] = classCounts[c] / data.length; });
...
// Transformamos multiplicação em SOMA
    ...
    classes.forEach(targetClass => {
      ...
      Object.entries(inputValues).forEach(([attrName, value]) => {
        if (value === '' || value === undefined) return;
        
        const attrIndex = attrKeys.indexOf(attrName);
        ...
        if (attr.type === 'numeric') {
          const allAttrValues = data.map(row => row[attrIndex]);
          processedValue = discretizeNumeric(value, allAttrValues);
        }
        
        const classData = data.filter(row => row[classIndex] === targetClass);
        const attrValues = classData.map(row => {
    ...
    Object.entries(results).forEach(([c, p]) => {
      normalizedProbs[c] = Math.exp(p - maxProb);
      sum += normalizedProbs[c];
    });
    
    Object.keys(normalizedProbs).forEach(c => {
      normalizedProbs[c] = (normalizedProbs[c] / sum * 100).toFixed(2);
    });
    
    const predictedClass = Object.entries(normalizedProbs)
  ...
  const handleInputChange = (attr, value) => {
    setInputValues(prev => ({ ...prev, [attr]: value }));
  };
  ...
  const currentAttributes = dataset === 'heart' ? heartAttributes : bankAttributes;
        ...
        <div className="bg-white rounded-2xl shadow-2xl overflow-hidden">
          <div className="bg-gradient-to-r from-blue-600 to-indigo-600 p-8 text-white">
            <div className="flex items-center gap-3 mb-2">
              ...
              <h1 className="text-4xl font-bold">Classificador Naive Bayes</h1>
            </div>
            <p className="text-blue-100 text-lg">
              Predição inteligente com atributos parciais ou completos
            </p>
          </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <ul className="text-blue-800 space-y-1 text-sm">
                    <li>• Selecione o dataset desejado</li>
                    <li>• Preencha todos ou apenas alguns atributos</li>
                    <li>• Clique em Classificar para ver a predição</li>
                    <li>• Quanto mais atributos, mais precisa a predição</li>
                  </ul>
                  <button 
          ...
          <div className="p-8">
            <div className="mb-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <button
                  onClick={() => { setDataset('heart'); resetForm(); }}
                ...
                >
                  <Heart className={`w-8 h-8 mx-auto mb-2 ${dataset === 'heart' ? 'text-red-500' : 'text-gray-400'}`} />
                  <div className="font-bold text-gray-800">Doenças Cardíacas</div>
                  <div className="text-sm text-gray-600">Predição de problemas cardíacos</div>
                </button>
                
                <button
                  onClick={() => { setDataset('bank'); resetForm(); }}
                  className={`p-4 rounded-xl border-2 transition-all ${
                    dataset === 'bank' 
                ...
                >
                  <Building2 className={`w-8 h-8 mx-auto mb-2 ${dataset === 'bank' ? 'text-green-500' : 'text-gray-400'}`} />
                  <div className="font-bold text-gray-800">Marketing Bancário</div>
                  <div className="text-sm text-gray-600">Predição de adesão a produtos</div>
                </button>
              </div>
            </div>
            ...
            <div className="bg-gray-50 rounded-xl p-6 mb-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {Object.entries(currentAttributes).map(([key, attr]) => (
                  <div key={key} className="bg-white p-4 rounded-lg shadow-sm">
                    ...
                    {attr.type === 'numeric' ? (
                      ...
                      >
                        ...
                        {Object.entries(attr.values).map(([val, label]) => (
            ...
            <div className="flex gap-4 mb-6">
              <button
                ...
                className="flex-1 bg-gradient-to-r from-blue-600 to-indigo-600 text-white py-4 px-6 rounded-xl font-bold text-lg hover:from-blue-700 hover:to-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-lg"
              >
                Classificar
              </button>
              <button
              ...
              <div className="bg-gradient-to-r from-green-50 to-emerald-50 border-2 border-green-300 rounded-xl p-6 shadow-lg">
                ...
                <div className="space-y-3">
                  ...
                  {Object.entries(probabilities).map(([cls, prob]) => (
                    <div key={cls} className="bg-white rounded-lg p-4 shadow-sm">
                      <div className="flex justify-between items-center mb-2">
                        <span className="font-semibold text-gray-700">
                          {dataset === 'heart' 
                            ? (cls === '0' ? 'Sem Doença: ' : `Nível ${cls}: `)
                            : (cls === 'yes' ? 'Aceita' : 'Recusa')
                          }
                        </span>
                        <span className="text-xl font-bold text-blue-600">{prob}%</span>
                      </div>
                ...
                <div className="mt-4 text-sm text-gray-600 bg-blue-50 p-3 rounded-lg">
                  <strong>Atributos:</strong> {Object.keys(inputValues).filter(k => inputValues[k]).length} de {Object.keys(currentAttributes).length}
                </div>
              </div>
            )}
          </div>
